...
/**
 * Express app with routes only. No Kafka consumer, no prom-client registration, no listen.
 * Pass `metrics` from startServer after histograms are registered on the Registry.
 */
export function createApp(
    redis: TrackingRedis,
    pool: TrackingPool,
    metrics?: { register: client.Registry; httpDuration: client.Histogram<string> }
): Express {
    const app = express();
    ...
    /** SCAN for `vehicle:*:latest` keys (avoids blocking Redis with KEYS). */
    async function scanVehicleLatestKeys(): Promise<string[]> {
        const keys: string[] = [];
        let cursor = '0';
        do {
            const [nextCursor, batch] = await redis.scan(cursor, 'MATCH', 'vehicle:*:latest', 'COUNT', 500);
            cursor = nextCursor;
            keys.push(...batch);
        } while (cursor !== '0');
        return keys;
...
app.get('/vehicles', async (req, res) => {
    try {
        const keys = await scanVehicleLatestKeys();
